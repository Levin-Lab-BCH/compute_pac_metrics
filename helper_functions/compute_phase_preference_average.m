%% Phase preference - tasks the raw amp_dist and generates phase preference matrices for individuals and group averages
function [phase_preference_avg,phase_matrices] = compute_phase_preference_average(alpha,amp_dist,comodulogram_row_headers,comodulogram_column_headers,comodulogram_third_dim_headers,groups)
...
phase_preference_avg.(groups{1,1})= zeros(length(comodulogram_row_headers),length(comodulogram_column_headers),length(comodulogram_third_dim_headers));
phase_preference_avg.(groups{1,2}) = zeros(length(comodulogram_row_headers),length(comodulogram_column_headers),length(comodulogram_third_dim_headers));
for hf = 1:length(comodulogram_row_headers)
    ...
    for lf = 1:length(comodulogram_column_headers)
        ...
        for ch = 1:length(comodulogram_third_dim_headers)
           ...
           phase_preference_avg.(groups{1,1})(hf, lf, ch) = circ_mean(squeeze(curr_group_1(~isnan(curr_group_1))));
            phase_preference_avg.(groups{1,2})(hf, lf, ch) = circ_mean(squeeze(curr_group_2(~isnan(curr_group_2))));
